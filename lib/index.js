...
BatchPayment.prototype = {
  ...
  fromArray(sourceSecret, paymentsArray) {
    const os = this.fromStream(sourceSecret, streamify(paymentsArray.concat([null])))
  ...
  fromStream(sourceSecret, paymentsStream) {
  ...
  fromCsv(sourceSecret, csvFilePath, csvOptions) {
    const readStream = fs.createReadStream(csvFilePath);
    return this.fromStream(sourceSecret, readStream.pipe(csv(csvOptions || {})))
  },
